...
class serSwitch(object):
    ...
    def __init__(self, addr = 0, portnum = 32):
        ...
        self.port = 0
        self.bitstat = ''
        self.remoteIP = '192.168.1.85'
        self.remotePort = 9001
        self.localIP = '192.168.1.11'
        self.localPort = 9001
    ...
    def buildCmd(self, cmd_type):
        ...
        cmd_type_dict = {
              ...
              'off':   self.cmdfuncoff,   # Turn off one switch
...
#            cmd_nochk = '%s%s%s\x00\x00\x3f\xff' % (self.cmdsendheader, struct.pack('B', self.addr), cmd_type_dict[cmd_type])
    ...
    def run(self, action, port=0, output=True):
        ...
        self.sock.send(self.buildCmd(action))
        if self.action == 'look':
            #curr_stat = self.chk_result(ser)
            curr_stat = self.chk_result()
            ...
            if self.chk_result(): return True
            else:
                if output == True:
...
#                    return False
    ...
    #def chk_result(self, ser):
        ...
        #    time.sleep(0.2)
        #    #inwait = ser.inWaiting()
        #    inwait = self.sock.recv(1000)
        #retn_data = ser.read(inwait)
        retn_data = self.sock.recv(1000)
        if retn_data == '':
