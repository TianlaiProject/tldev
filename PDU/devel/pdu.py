...
class pdu():
	...
	self.command={	'current': '1.3.6.1.4.1.318.1.1.12.2.3.1.1.2.1',
			...
			'outlet_header':'1.3.6.1.4.1.318.1.1.12.3.3.1.1.4.',
		 	...
		 	'name_N': '1.3.6.1.4.1.318.1.1.4.5.2.1.3.',
    ...
    def action(self, outlet, action, IP = None):
        ...
        outlet=str(outlet)
        pMod.apiPDU.setVarBinds(reqPDU, ((self.command['outlet_header']+outlet, pMod.Integer(self.action_dict[action])), ))
        ...
        transportDispatcher.sendMessage(encoder.encode(reqMsg), udp.domainName, (IP, 161))
...
if __name__=='__main__':
    ...
    if options.port == [13, 'offi']: # when no input in command line
       pass
    else:
        pdu88.action(int(options.port[0]), options.port[1])
